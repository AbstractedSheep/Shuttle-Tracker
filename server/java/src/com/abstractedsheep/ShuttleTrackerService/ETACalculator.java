...
public class ETACalculator {
	...
	public class Eta {
		 ...
		 * @param stopId
		 */
		public Eta(int shuttleId, int routeId, int time, String stopId, String stopName, int etaId) {
	...
	public ArrayList<Eta> getETAs() { return this.etaList; }
	...
	private void calculatateETAs() {
		...
		for(Shuttle shuttle : world.getShuttleList().values()) {
			...
			for(Stop stop : rt.getStopList().values()) {
				...
				int j;
				double distance;
                int stopPrecedingCoordinate = stop.getPrecedingCoordinate().get(rt.getIdNum());
				...
				{
					distance = shuttle.getCurrentLocation().distanceFromCoordiante(shuttle.getClosestPoint());
				}
				else
				{
					...
					distance = shuttle.getCurrentLocation().distanceFromCoordiante(
									(rt.getCoordinateList().get(shuttle.getNextRouteCoordinate())));
                    ...
                    // will go through
					distance += stop.getPrecedingCoordinateDistance().get(rt.getIdNum());
				}
				int time = (int) (distance / shuttle.getSpeed());
				this.etaList.add(new Eta(shuttle.getShuttleId(), shuttle.getCurrentRoute().getIdNum(), time,
						stop.getShortName(), stop.getName(), 0));
			}
		}
    }
